...
int main(int argc, char* argv[]) {
  ...
  size_t pitch;
  // allocations
  assert(cudaMallocPitch(&d_data, &pitch, width * sizeof(int), height) == CUDA_SUCCESS);
  ...
  assert(cudaStreamCreate(&stream1) == CUDA_SUCCESS);
  cudaStream_t stream2;
  assert(cudaStreamCreate(&stream2)== CUDA_SUCCESS);
